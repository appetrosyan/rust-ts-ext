...
** Unique Functions
- [X] ~rust-ts-ext-doc-panics~: Inside a function item, highlight all of the unwrap calls, and add the text of `expect` conditions to the ~# Panics~ section of the function's documentation.
- [X] =rust-ts-ext-insert-default=: Inside a struct item, looking at its identifier, insert an ~impl Default~ based on the following heuristics.  If a ~Self::new~ is defined and takes no arguments, just replace with a call to that.  If it takes any arguments, replace those arguments with ~Default::default()~.  If no ~Self::new~ is defined, just replace every member with ~Default::default~ (which is what ~derive(Default)~ does usually.
- [X] =rust-ts-ext-insert-anonymous-lifetime=: Saves you a bunch of typing.
...
- [X] ~rust-ts-ext-insert-ok-dwim~
- [X] =rust-ts-ext-insert-impl-block-dwim=
- [ ] ~rust/insert-struct~
- [ ] ~rust/insert-enum~
- [ ] ~rust/insert-mod~
- [ ] ~rust/insert-derive~
...
There is
- [ ] ~rust/convert-tuple~
- [ ] =rust-ts-ext-try-from=: Replace =as= casts with =try_from= implementations for numerical operations.
- [ ] ~rust/convert-struct~
...
- [X] =rust-ts-ext-cargo-add=
- [X] =rust-ts-ext-cargo-clippy=
- [X] =rust-ts-ext-cargo-clippy=
- [X] ~rust-ts-ext-cargo-check~
- [ ] ~rust/cargo-delete-lock~
- [X] ~rust-ts-ext-open-cargo-toml~
- [X] ~rust-ts-ext-open-workspace-cargo-toml~
...
** TODO Add way to bulk add workspace inheritance to crates

** TODO Add way to extract any given structure into its own module

** TODO Add way to convert module to crate
